"""
Models for satellite greenery segmentation.

Two architectures are provided:

  UNet (ViT) — pure Vision Transformer.
      Patch-embeds the 64x64 image into 64 tokens, processes them with 6
      Transformer blocks, then progressively upsamples the token grid back
      to full resolution. No spatial inductive bias.

  TransUNet  — hybrid Transformer + U-Net (active model).
      CNN encoder builds hierarchical spatial features at 4 scales.
      A Transformer bottleneck applies global self-attention over the 8x8
      feature map (64 tokens, dim=512) before decoding.
      A U-Net decoder then restores full resolution with skip connections
      from each encoder stage for precise spatial localisation.
      Combines CNN inductive bias (locality, translation equivariance)
      with Transformer global context at the bottleneck.
"""
...
class PatchEmbed(nn.Module):
    ...
    Uses a Conv2d with kernel_size=stride=patch_size, which is equivalent to
    extracting patches and applying a shared linear projection.
    """
...
class UNet(nn.Module):
    """Vision Transformer (ViT) for binary segmentation of satellite imagery.

    Architecture:
        Patch Embedding:      (B, 3, 64, 64) -> (B, 64 patches, embed_dim)
        ...
        Segmentation Decoder: reshape patch tokens -> 3x 2x conv upsampling
        Output:               (B, 1, 64, 64) — per-pixel greenery probability

    Unlike the CNN U-Net, the self-attention layers model global spatial
    relationships across all 64 patches simultaneously, enabling long-range
    context when predicting greenery masks.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Decode back to full image resolution
...
class TransformerBottleneck(nn.Module):
    ...
    (B, C, H, W). Channel dimension C is preserved throughout so no projection
    is needed between the CNN encoder and this module.
...
class TransUNet(nn.Module):
    """Hybrid Transformer + U-Net for satellite greenery segmentation.

    Architecture:
        CNN Encoder:
            enc1: ConvBlock(3  ->  64)          64x64  — fine texture / colour
            enc2: Pool + ConvBlock(64  -> 128)  32x32  — mid-level features
        ...
        Transformer Bottleneck:
            ...
            Global self-attention gives every bottleneck location access to
            all others before upsampling — unlike a plain U-Net bottleneck.
    ...
    The skip connections preserve fine spatial detail from the encoder while
    the transformer bottleneck captures long-range context — combining the
    strengths of both CNN and Transformer designs.
...
if __name__ == "__main__":
    ...
    vit = UNet(in_channels=3, out_channels=1)
    print(f"ViT (UNet) Parameters:   {count_parameters(vit):,}")

    hybrid = TransUNet(in_channels=3, out_channels=1)
    out = hybrid(dummy)
    print(f"TransUNet Parameters:    {count_parameters(hybrid):,}")
