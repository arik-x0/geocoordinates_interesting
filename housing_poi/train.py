"""
Training script for the HousingEdgeCNN structure detection submodel.

Loads the frozen CoreSatelliteModel (EVA-02 ViT-S/14) to extract
multi-scale features from each RGB tile, then trains the lightweight
HousingEdgeCNN submodel on those features using NDBI pseudo-labels.
...
--- Loading Core Model (EVA-02 ViT-S/14, frozen) ---")
    ...
    print(f"  HousingEdgeCNN submodel (trainable): {count_parameters(submodel):,} params")
...
")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if is_best:
            ...
            torch.save({
                "epoch": epoch, "submodel_state_dict": submodel.state_dict(),
                "optimizer_state_dict": optimizer.state_dict(),
                "val_iou": val_iou, "val_dice": val_dice, "val_loss": val_loss,
            }, checkpoint_dir / "best_model.pth")
            print(f"  ** New best model saved (IoU: {val_iou:.4f}) **")
        epoch_logs.append({
            "epoch": epoch, "timestamp": datetime.now().strftime("%Y-%m-%dT%H:%M:%S"),
            "epoch_duration_s": round(epoch_duration, 2), "elapsed_s": round(elapsed, 2),
            "train_loss": round(train_loss, 6), "train_iou": round(train_iou, 6),
            "train_dice": round(train_dice, 6), "val_loss": round(val_loss, 6),
            "val_iou": round(val_iou, 6), "val_dice": round(val_dice, 6),
    ...
    torch.save({
        "epoch": args.epochs, "submodel_state_dict": submodel.state_dict(),
        "optimizer_state_dict": optimizer.state_dict(),
    }, checkpoint_dir / "final_model.pth")
    print(f"Models saved to: {checkpoint_dir}")
    build_embedding_index(core.encode, [train_loader, val_loader, test_loader], device, checkpoint_dir)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train housing edge detection submodel on frozen EVA-02 core features")
