"""SublimePredictor: ranks tiles by large-scale contrast / sublime score."""
...
from dataset import get_sublime_dataloaders as _get_loaders         # noqa: E402

from .model import ScaleSublimeNet
from .utils import compute_sublime_score, visualize_sublime


class SublimePredictor(BasePredictor):
    score_key = "sublime_score"
    ...
    def build_submodel(self):
        return ScaleSublimeNet(out_channels=1)
    ...
    def build_result(self, i, inputs_cpu, targets_cpu, preds_cpu, metas, embs_cpu):
        return {
            ...
            "sublime_score":  compute_sublime_score(preds_cpu[i, 0]),
    ...
    def print_ranking(self, all_results, top_n):
        n      = len(all_results)
        scores = [r["sublime_score"] for r in all_results]
        print(f"\n{'='*70}")
        print(f"  SCALE & SUBLIME RANKING -- Top {min(top_n, n)} of {n}")
        print(f"{'='*70}")
        print(f"  {'Rank':<6} {'Score':<10} {'Class':<22} File")
        print(f"  {'-'*6} {'-'*10} {'-'*22} {'-'*28}")
        for rank, r in enumerate(all_results[:top_n], 1):
            print(f"  {rank:<6} {r['sublime_score']:<10.4f} "
                  f"{r['class_name']:<22} {Path(r['filepath']).name}")
        print(f"\n  Mean: {np.mean(scores):.4f}  Max: {np.max(scores):.4f}")
        class_scores: dict = {}
        for r in all_results:
            class_scores.setdefault(r["class_name"], []).append(r["sublime_score"])
        print(f"\n  {'Class':<25} {'Avg':>8}  Bar")
    ...
    def save_visualizations(self, all_results, output_dir, top_n):
        print(f"\nSaving top-{min(top_n, len(all_results))} sublime visualizations...")
        for rank, r in enumerate(all_results[:top_n], 1):
            visualize_sublime(
                rgb=r["rgb"], label_true=r["label_true"], label_pred=r["label_pred"],
                score=r["sublime_score"],
                save_path=str(output_dir / f"sublime_{rank:02d}_{r['class_name']}.png"),
            )
